...
- **Body**: card list grouped by type below the sticky header. **Main grid orientation**: characters use landscape `380:280` with thumbnail + cover; locations use **`502:359`** with full-res + cover; events use `236:151` with full-res + cover; all other types use portrait `5:7` with thumb + contain. Landscape sections use fixed **`285px`** columns (`repeat(auto-fill, 285px)`); portrait groups use `minmax(210px, 1fr)`. **Mobile (`.layout-mobile`)**: main deck body and Add Cards grids use **one card per row** (`grid-template-columns: minmax(0, 1fr)`) for all catalog types — full-width tiles, no multi-column cramming. **Mobile type tabs**: horizontal chip strip (Add Cards `shortLabel` style) lists **only types present in the deck**; one type visible at a time; section titles hidden when tabs show. **Swipe left/right** on the card list cycles types cyclically (`useHorizontalSwipe` with native touch listeners on `.deck-editor__content`; swipes work on card art, blocked on footer/tabs/header).
