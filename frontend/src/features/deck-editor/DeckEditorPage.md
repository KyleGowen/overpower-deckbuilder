...
- **Sticky page header** (`.deck-editor__header`): on **desktop**, stays pinned while the card list scrolls. On **mobile**, the header, type tabs, and card list share one scroll container (`.deck-editor__main`) so metadata scrolls off-screen for more card space. Single **topbar row** (`.deck-editor__topbar`) with three zones:
  ...
  - **Center** (`.deck-editor__stats-panel`): **Threat** `StatIconBadge` (`lg`) left of **Max** (character maximum primaries) and **Total** (deck-wide icon counts via `calculateDeckIconTotals`) — each stat row with a short uppercase label (`.deck-editor__stats-block-label`: **Max** / **Total**) and four `StatIconBadge` overlays. Header stat icons are **90%** of `lg` (36px) on desktop; card-tile icons stay full `lg`. **Desktop narrow main column** (`@container deck-editor-main` ≤1700px): topbar becomes two rows — row 1 keeps back, name, and meta chips on one line plus actions; row 2 is a full-width centered stats panel (see `STYLE_GUIDE_V2.md` § Deck Editor — Desktop header compaction). Section and per-icon `title` tooltips still say “Character max” / “Icon total” for clarity.
  ...
  - **Mobile**: stats use subgrid alignment; threat icon stat lives in the **meta row** (same horizontal line as card count + legality, threat right-aligned via `margin-left: auto`); Max and Total rows below; action buttons use compact chip-sized pills (`4px 10px`, `font-size-xs`); actions full-width below stats
...
- **Wide desktop context pane (`>1200px`)**: left side of the expanded drawer, split into fixed-height working regions:
  - **Team stats (top 20%)**: four rows, one per character slot. Filled rows show character name plus readable generated stat badges for Energy, Combat, Brute Force, Intelligence, and Threat Value. Empty team slots render subtle placeholder rows. Decks should never exceed four character rows. Hovering a filled team row previews that selected character printing in the image pane.
  - **Compact filters (middle 15%)**: the existing Set and Hide Unusables controls plus type-specific filters for the active right-pane card type. Filters reuse DBV filter behavior in a smaller surface: numeric stats for characters/power/locations (location Threat Value included), power-type strips for universe-style cards, function icons for specials/advanced universe, and mission-set filters for missions/events. These controls wrap within the context pane without horizontal scrolling, and sit in the results pane below 1200px.
  - **Hover preview (bottom 65%)**: image-only full-card preview using `CardImage`. Hovering a card in the results pane, or a filled team row, shows the selected card image scaled to fit the pane; clicking a result card still adds it. The preview stays on the last hovered card after add, and clears when the pointer leaves a hovered card/team row.
- **Filter persistence**: Add Cards search, active type tab, Set, Hide Unusables, current page, and dynamic type filters persist while opening/closing Add Cards in the same deck-editor session. Dynamic type filters are cached per card type, so tabbing away and back restores that type's selections. They reset when leaving/reloading the deck editor.
