...
function formatServiceCurrency(value: number, currency: string): string {
  return value > 0 && value < 0.01
    ? `<${formatCurrency(0.01, currency)}`
  ...
  return (
    <Card className="biz-ops-service-trend-card">
      <div className="biz-ops-service-trend-heading">
        ...
        <strong>{formatServiceCurrency(trend.currentAmount, currency)}</strong>
      </div>
      <div
        className="biz-ops-service-trend-chart"
        role="img"
        aria-label={`${trend.service} monthly AWS invoice row history, with a current estimate of ${formatCurrency(trend.currentAmount, currency)}.`}
      >
              ...
              content={({ active, payload }) => {
                ...
                return active && entry ? (
                  <div className="biz-ops-chart-tooltip biz-ops-service-chart-tooltip">
                    ...
                    <b>{formatCurrency(entry.amount, currency, 4)}</b>
                  </div>
                ) : null;
              }}
            />
      ...
      <article className="biz-ops-dashboard">
        <header className="biz-ops-hero">
          ...
          <Card className="biz-ops-ytd-card">
            ...
            <strong>{formatCurrency(analytics.yearToDate.trackedTotal, analytics.currency)}</strong>
            <small>
              {formatCurrency(analytics.yearToDate.finalizedTotal, analytics.currency)} final +{' '}
              {formatCurrency(analytics.yearToDate.estimatedTotal, analytics.currency)} estimated
            </small>
          </Card>
        </header>
        ...
        <section className="biz-ops-kpis" aria-label="AWS cost summary">
          <Card className="biz-ops-kpi-card">
            ...
            <strong>{formatCurrency(analytics.currentMonth.estimatedTotal, analytics.currency)}</strong>
            <p>
          ...
          <Card className="biz-ops-kpi-card">
            ...
            <strong>{formatCurrency(analytics.currentMonth.previousFinalizedTotal, analytics.currency)}</strong>
            <p>
          ...
          <Card className="biz-ops-kpi-card">
            ...
            <strong>{formatCurrency(analytics.currentMonth.dailyAverage, analytics.currency)}</strong>
            <p>
              Simple month-end pace: <b>{formatCurrency(analytics.currentMonth.projectedTotal, analytics.currency)}</b>
        ...
        <section className="biz-ops-main-grid">
          <Card className="biz-ops-panel biz-ops-trend-panel">
            ...
            <div
              ...
              role="img"
              aria-label={`Monthly finalized AWS costs through ${formatMonth(analytics.currentMonth.previousFinalizedMonth)}, plus an estimate of ${formatCurrency(analytics.currentMonth.estimatedTotal, analytics.currency)} through ${formatDay(analytics.currentMonth.throughDate)}.`}
            >
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 20, right: 12, left: 2, bottom: 4 }}>
                  ...
                  <YAxis
                    ...
                    tickCount={5}
                    tickFormatter={(value: number) => formatCurrency(value, analytics.currency, 0)}
                    axisLine={false}
                    tickLine={false}
                    width={56}
                    tick={{ fill: 'var(--color-text-muted)', fontSize: 12 }}
                    ...
                    content={({ active, payload }) => {
                      ...
                      return active && entry ? (
                        <div className="biz-ops-chart-tooltip">
                          ...
                          <b>{formatCurrency(entry.amount, analytics.currency)}</b>
                ...
                return (
                  ...
                  >
                    <div className="biz-ops-service-meta">
                      ...
                      <span>{formatServiceCurrency(service.amount, analytics.currency)} · <b>{service.percentage.toFixed(1)}%</b></span>
                    </div>
                    ...
                    <div className="biz-ops-service-tooltip" id={tooltipId} role="tooltip">
                      ...
                      <span>{formatCurrency(service.amount, analytics.currency, 4)} month to date</span>
        ...
        <footer className="biz-ops-footer">
          <div>
            ...
            {analytics.latestWeeklyDigest ? (
              <span>
                Latest weekly digest: {formatCurrency(analytics.latestWeeklyDigest.amount, analytics.currency)} ·{' '}
