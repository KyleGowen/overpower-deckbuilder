-- Add second alternate art for King Arthur (ERB)
...
BEGIN
  ...
  IF base_count <> 1 THEN
    RAISE EXCEPTION 'Expected exactly 1 base character row for King Arthur (ERB), found %', base_count;
  END IF;
END $$;
...
SELECT
  ...
  c.set_number_foil
FROM characters c
WHERE c.name = 'King Arthur'
  AND c.set = 'ERB'
  AND (c.image_path NOT LIKE '%/alternate/%' OR c.image_path IS NULL)
  AND NOT EXISTS (
    SELECT 1 FROM characters x
    WHERE x.name = c.name AND x.set = c.set AND x.image_path = 'characters/alternate/king_arthur2.png'
  )
LIMIT 1;
