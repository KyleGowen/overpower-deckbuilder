...
// ----- Guest deck API (GUEST role only; session-scoped, not persisted to DB) -----
...
app.get('/api/guest/decks', authenticateUser, async (req: Request, res) => {
  try {
    ...
    // Preloaded guest decks: DB decks owned by the guest user (from migrations)
    const dbDecks = await deckRepository.getDecksByUserId(req.user!.id);
    const dbTransformed = transformDeckList(dbDecks);

    // Session-scoped guest decks (created this session via +Deck)
...
app.post('/api/guest/decks/:id/cards', authenticateUser, async (req: Request, res) => {
  try {
    ...
    const validationError = await validateCardAddition(currentCards, cardType, cardId, qty);
    if (validationError) {
    ...
    if (isCataclysm) {
      ...
      for (const c of currentCards) {
        if (await checkIfCardIsCataclysm(c.type, c.cardId)) {
          hasExisting = true;
          break;
        }
      }
