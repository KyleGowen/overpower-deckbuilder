...
new DeckService(deckRepository);
...
// Guest deck persistence (session-scoped, for integration tests)
const guestDeckPersistence = new GuestDeckPersistenceService();

function requireGuestSession(req: Request, res: import('express').Response): string | null {
...
function transformGuestDeckToListItem(deckData: DeckData) {
  return {
    metadata: {
...
const optionalAuth = async (req: Request, res: import('express').Response, next: import('express').NextFunction) => {
  try {
...
let testServer: import('http').Server | null = null;
...
app.get('/api/debug/clear-cache', authenticateUser, async (req: Request, res) => {
  try {
    if (!requireAdmin(req, res)) return;
    (deckRepository as { clearCache?: () => void }).clearCache?.();
...
// Deck routes
app.get('/api/decks', authenticateUser, async (req: Request, res) => {
  try {
...
app.get('/api/decks/:id', optionalAuth, async (req: Request, res) => {
  try {
...
// Guest deck API (GUEST role only; session-scoped)
app.post('/api/guest/decks', authenticateUser, async (req: Request, res) => {
  try {
    ...
    if (name.length > 100) return res.status(400).json({ success: false, error: 'Deck name must be 100 characters or less' });
    if (description.length > 500) return res.status(400).json({ success: false, error: 'Description must be 500 characters or less' });
    const now = new Date().toISOString();
    const deckData: DeckData = {
      metadata: { id: '', name, description, created: now, lastModified: now, cardCount: 0, userId: sessionId },
      cards: []
    };
    const deckId = guestDeckPersistence.createDeck(sessionId, deckData);
    const created = guestDeckPersistence.getDeck(sessionId, deckId)!;
    res.status(201).json({ success: true, data: { id: deckId, name: created.metadata.name, description: created.metadata.description ?? '', created_at: created.metadata.created, updated_at: created.metadata.lastModified } });
  } catch (error) {
    console.error('Error creating guest deck:', error);
    res.status(500).json({ success: false, error: 'Failed to create guest deck' });
  }
});
app.get('/api/guest/decks', authenticateUser, async (req: Request, res) => {
  try {
    const sessionId = requireGuestSession(req, res);
    if (!sessionId) return;
    const dbDecks = await deckRepository.getDecksByUserId(req.user!.id);
    const dbTransformed = transformDeckList(dbDecks);
    const sessionDecks = guestDeckPersistence.getAllDecksForSession(sessionId);
...
app.get('/api/guest/decks/:id', authenticateUser, async (req: Request, res) => {
  try {
    const sessionId = requireGuestSession(req, res);
    if (!sessionId) return;
    const deckData = guestDeckPersistence.getDeck(sessionId, req.params.id);
    if (!deckData) return res.status(404).json({ success: false, error: 'Deck not found' });
    res.json({ success: true, data: { metadata: { ...deckData.metadata, isOwner: true }, cards: deckData.cards || [] } });
  } catch (error) {
    console.error('Error fetching guest deck:', error);
    res.status(500).json({ success: false, error: 'Failed to fetch guest deck' });
  }
});
app.put('/api/guest/decks/:id', authenticateUser, async (req: Request, res) => {
  try {
    const sessionId = requireGuestSession(req, res);
    if (!sessionId) return;
    const existing = guestDeckPersistence.getDeck(sessionId, req.params.id);
    ...
    if (name.length > 100) return res.status(400).json({ success: false, error: 'Deck name must be 100 characters or less' });
    if (description && description.length > 500) return res.status(400).json({ success: false, error: 'Description must be 500 characters or less' });
    const updated: DeckData = { metadata: { ...existing.metadata, name, description: description ?? '', lastModified: new Date().toISOString() }, cards: existing.cards };
    const ok = guestDeckPersistence.updateDeck(sessionId, req.params.id, updated);
    if (!ok) return res.status(404).json({ success: false, error: 'Deck not found' });
    const result = guestDeckPersistence.getDeck(sessionId, req.params.id)!;
...
app.put('/api/guest/decks/:id/cards', authenticateUser, async (req: Request, res) => {
  try {
    const sessionId = requireGuestSession(req, res);
    if (!sessionId) return;
    const existing = guestDeckPersistence.getDeck(sessionId, req.params.id);
    ...
    if (cards.length > 100) return res.status(400).json({ success: false, error: 'Cannot replace more than 100 cards at once' });
    ...
    const updated: DeckData = { metadata: { ...existing.metadata, lastModified: new Date().toISOString(), cardCount }, cards: mappedCards };
    const ok = guestDeckPersistence.updateDeck(sessionId, req.params.id, updated);
    if (!ok) return res.status(404).json({ success: false, error: 'Deck not found' });
    const result = guestDeckPersistence.getDeck(sessionId, req.params.id)!;
...
app.post('/api/guest/decks/:id/cards', authenticateUser, async (req: Request, res) => {
  try {
    const sessionId = requireGuestSession(req, res);
    if (!sessionId) return;
    const existing = guestDeckPersistence.getDeck(sessionId, req.params.id);
    ...
    const updated: DeckData = {
      metadata: { ...existing.metadata, lastModified: new Date().toISOString(), cardCount },
      cards: updatedCards
    };
    const ok = guestDeckPersistence.updateDeck(sessionId, req.params.id, updated);
    if (!ok) return res.status(404).json({ success: false, error: 'Deck not found' });
...
app.delete('/api/guest/decks/:id', authenticateUser, async (req: Request, res) => {
  try {
    const sessionId = requireGuestSession(req, res);
    if (!sessionId) return;
    const ok = guestDeckPersistence.deleteDeck(sessionId, req.params.id);
    if (!ok) return res.status(404).json({ success: false, error: 'Deck not found' });
...
app.put('/api/decks/:id', optionalAuth, async (req: Request, res) => {
  try {
...
app.post('/api/decks/:id/cards', authenticateUser, async (req: Request, res) => {
  try {
    ...
    type LocalDeckCard = { type: string; cardId: string; quantity: number };
    const validateCardAddition = async (currentCards: LocalDeckCard[], cardType: string, cardId: string, quantity: number): Promise<string | null> => {
      ...
      const characterCards: LocalDeckCard[] = [];
      const missionCards: LocalDeckCard[] = [];
      const locationCards: LocalDeckCard[] = [];
      ...
      const cataclysmCards: LocalDeckCard[] = [];
      for (const card of testCards) {
      ...
      const assistCards: LocalDeckCard[] = [];
      for (const card of testCards) {
      ...
      const ambushCards: LocalDeckCard[] = [];
      for (const card of testCards) {
      ...
      const fortificationCards: LocalDeckCard[] = [];
      for (const card of testCards) {
...
app.post('/api/collections/me/cards', authenticateUser, async (req: Request, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: (error as { message?: string })?.message || 'Failed to add card to collection' });
  }
});
...
app.post('/api/collections/me/cards/remove-one', authenticateUser, async (req: Request, res) => {
  try {
    ...
    res.json({ success: true, data: updatedCard, message: 'One copy removed from collection' });
  } catch (error) {
...
app.get('/deck-editor/:deckId', optionalAuth, async (req: Request, res) => {
