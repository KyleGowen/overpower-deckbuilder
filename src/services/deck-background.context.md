...
| Role | Path |
|------|------|
| **Background images** | `src/resources/cards/images/backgrounds/` — only `.png` files; **auto-discovered** (no manifest). |
...
Paths are **relative to project root**, e.g. `src/resources/cards/images/backgrounds/moriartyreapernotext.png`. The frontend uses them as `/${path}` (e.g. `/src/resources/cards/images/backgrounds/…`) for `url()`.

---

## Backend: DeckBackgroundService

- **`getAvailableBackgrounds()`** — `readdir` of `src/resources/cards/images/backgrounds`, filter `.png`, return sorted paths; cached 15 minutes.
...
No thumbnails: deck backgrounds are full-size images. The card thumbnail pipeline (`generateCardThumbnails.ts`) does **not** include the `backgrounds/` directory.
...
1. Add a `.png` file to `src/resources/cards/images/backgrounds/`.
...
See `.cursorrules` in `src/resources/cards/images/backgrounds/` for directory-specific rules.
...
Integration tests expect paths like `src/resources/cards/images/backgrounds/<name>.png` and that the API returns only paths under that directory.
