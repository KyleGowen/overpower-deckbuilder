...
- A second full-width analytics band pairs two equal bordered cards: cumulative site-section API request-share bars for Home, Database, Decks, and Collection, and a 24-point Pacific-time login radar chart. The usage card must say it is request share rather than time, page views, or unique users, and the radar chart must state that it covers the rolling last 24 hours.
- The login radar is arranged as a clock: midnight starts at the top, hours progress clockwise, and major labels appear every three hours while all 24 hourly points remain plotted. Cyan stroke, translucent fill, and visible point markers sit over circular polar grid lines; exact hourly counts appear on hover. Backend grouping uses the DST-aware `America/Los_Angeles` timezone.
