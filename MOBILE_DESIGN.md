...
| Global nav              | `[public/components/globalNav.css](public/components/globalNav.css)`, `[public/css/mobile-layout.css](public/css/mobile-layout.css)` | `@media` at 900px / 600px; under `.layout-mobile`, **CSS grid** on `**unified-header**` (**`auto 1fr auto`**: logo | centered tabs | user); **`header-nav-cluster`** **`display: contents`**; **`.user-menu-toggle`** **`min-height: 44px`** — see **§10**. Desktop: **`header-nav-cluster`** **`display: contents`** (unchanged). |
...
- **User row:** `**#userMenu**` `**max-width: min(160px, 42vw)**`; `**.user-menu-toggle`** `**justify-content: flex-end**`, `**font-size: 14px**`; **`.user-greeting`** is a **row**: **`.user-greeting-copy`** stacks **Welcome,** / **`#currentUsername`**! with **full-width right alignment** so line endings share a vertical edge; **`.triangle-icon`** (**`16px`**) sits to the **right of the copy**, vertically centered; **`#currentUsername`** **ellipsis** when long.
