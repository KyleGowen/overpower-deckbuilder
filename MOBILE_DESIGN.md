...
| Collection              | `[public/css/collection-view.css](public/css/collection-view.css)`, `[public/js/collection-view.js](public/js/collection-view.js)`     | **DTV:** single wide table + column resize. **MV (`layout-mobile`):** list rows (thumb + title + type/#/set subtitle + qty stepper), set **#** sort only; add-cards search is **`#collectionSearchInput`** only; bottom-sheet detail (`#collectionMobileDetail`); `layout-mode-change` re-renders.                                                                                                                                                 |
