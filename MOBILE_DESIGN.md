...
- **2×2 grid:** `**#newDeckBtn**` lives in `**.header-right**`. Under `**.layout-mobile**`, `**mobile-layout.css**` sets `**display: grid**` `**1fr 1fr**` on `**.header-nav-cluster**` and `**display: contents**` on `**.header-center**`, `**.header-app-actions**`, `**.app-tabs**`, and `**.header-right**` so four buttons share one grid. Placement: row 1 **Card Database**  **Collection**; row 2 **Deck Builder**  **+ Deck**; row 3 `**#userMenu**` full width (IDs `**#databaseViewBtn`**, `**#collectionViewBtn`**, `**#deckBuilderBtn`**, `**#newDeckBtn**`).
- **Logged out / no `getCurrentUser`:** `**[syncHeaderCollectionLayout()](public/components/globalNav.js)`** hides Collection and adds `**.collection-tab-hidden**` on `**.header-nav-cluster**` so **Card Database** spans the top row (no empty cell). Called from `**updateUserWelcome()`** after greeting/menu updates.
...
- **Unit:** `[tests/unit/layout-mode-and-viewport.test.ts](tests/unit/layout-mode-and-viewport.test.ts)` — `layout-mode.js`, `**mobile-layout.css`**: global nav + DBV **All** strip (M1 describe), **DBV Characters tab**, **DBV Special Cards tab** (block `**thead`**, flex filter `**tr**`, `**width: 100% !important**` on filter `**th**`, `**order**` rows, value sheet + function-row Clear, tbody cards); `**public/index.html**` asserts `**special-cards-filter-row**` and `**clear-special-filters-mobile**`. **Aspects** mobile DBV: [tests/unit/dbv-aspects-mobile.test.ts](tests/unit/dbv-aspects-mobile.test.ts) (CSS, `database-view.css`, markup, `displayAspects` source + JSDOM). **Missions** mobile DBV: [tests/unit/dbv-missions-mobile.test.ts](tests/unit/dbv-missions-mobile.test.ts) (see [docs/current/DBV_MISSIONS_MOBILE.md](docs/current/DBV_MISSIONS_MOBILE.md)). **Universe: Basic** mobile DBV: [tests/unit/dbv-basic-universe-mobile.test.ts](tests/unit/dbv-basic-universe-mobile.test.ts) (see [docs/current/DBV_BASIC_UNIVERSE_MOBILE.md](docs/current/DBV_BASIC_UNIVERSE_MOBILE.md)).
