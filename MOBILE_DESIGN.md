...
| Collection              | `[public/css/collection-view.css](public/css/collection-view.css)`, `[public/js/collection-view.js](public/js/collection-view.js)`     | **DTV:** single wide table + column resize. **MV (`layout-mobile`):** list rows (thumb + title + type/#/set subtitle + qty stepper), sort `<select>`, bottom-sheet detail (`#collectionMobileDetail`) with full art + duplicate qty; `layout-mode-change` re-renders.                                                                                                                                                 |
| Deck selection          | `[public/css/deck-selection.css](public/css/deck-selection.css)`                                                                     | 44×44 menu pattern in places.                                                                                                                                                                 |
| Global nav              | `[public/components/globalNav.css](public/components/globalNav.css)`, `[public/css/mobile-layout.css](public/css/mobile-layout.css)` | `@media` at 900px / 600px; under `.layout-mobile`, **two-row grid** on `**unified-header**` (row 1: logo | user menu; row 2: three equal view tabs) + 44px targets — see **§10**. Desktop: `**header-nav-cluster`** is `display: contents`. |
...
**M4:** **`collectionIsLayoutMobile()`** (`**window.isLayoutMobile()**`) chooses markup in **`displayCollectionCards`** — **DTV** unchanged (table + header sort + resize). **MV:** **`#collection-mobile-list`** of **`li.collection-mobile-row`** (grid: **56px** thumb, text, actions); row qty **`−`/`+`** **~29×29px**; detail sheet steppers **~32×32px** (~⅔ of earlier 44/48px for density); subtitle **`Type · #… · Set`**. **GUEST sandbox:** **`details#guestSandboxBanner`** — muted SVG warning icon + collapsed one-line summary + chevron; expand for full copy + **Create an account** (DTV stays always-open flex row). **`#collectionMobileListFilter`** (in **`.collection-mobile-toolbar`**) is a single catch-all text filter (display name, **special** **`character_name`**, type/set/**#**); list is always sorted **set # low → high** on MV; DTV header sort only affects the table. **Detail:** **`#collectionMobileDetail`** appended under **`#collection-view`** — scrim + **`#collectionMobileDetailPanel`** ( **`role="dialog"`** ), **Back** / **Escape** / scrim close; **`openCollectionMobileDetail`** / **`closeCollectionMobileDetail`** on **`window`** for tests. Row tap opens detail (buttons use **`stopPropagation`**). **`layout-mode-change`** closes detail and re-renders when the Collection tab shell is visible. **CSS:** **`html.layout-mobile`** rules in **`collection-view.css`** (toolbar, list filter input, list, **`z-index: 10000`** overlay). **Unit:** **`tests/unit/collection-view.test.ts`** (`**displayCollectionCards() mobile layout**`, **`mergedCollectionCardMatchesMobileListFilter**`, **`sortMergedCollectionCards**`).
