...
- **How it looks (filters, caption, no Clear, desktop checkboxes):** [`docs/current/DBV_MISSIONS_MOBILE.md`](docs/current/DBV_MISSIONS_MOBILE.md). **Regression tests:** [`tests/unit/dbv-missions-mobile.test.ts`](tests/unit/dbv-missions-mobile.test.ts).
- **Filter shell:** **`tr.missions-filter-row`** — teal border shell like Aspects; **mobile** shows **`.missions-mobile-set-row`** only (**label** **`#4ecdc4`** + **`#missions-mission-set-filter`**); **checkboxes hidden** on mobile. **Desktop:** **`#missions-table .missions-mobile-set-row { display: none }`**, **`missions-filter-clear-th` hidden** (no Missions-tab Clear column).
