...
- **Automated (unit):** [`tests/unit/layout-mode-and-viewport.test.ts`](../../tests/unit/layout-mode-and-viewport.test.ts) covers:
  ...
  - `mobile-layout.css`: global nav under `.layout-mobile` — **grid** `.unified-header` (**`repeat(3, minmax(0, 1fr))`**, two rows: logo | user menu, then three equal view tabs); **`.header-nav-cluster`** and inner wrappers use `display: contents`; **`#newDeckBtn`** hidden; **`.user-menu-toggle`** `justify-content: flex-end`, `width: auto`; **`.user-menu-dropdown`** half width (`50%`, `right: 0`); 44px targets on tabs / menu toggle. See [`MOBILE_DESIGN.md`](../../MOBILE_DESIGN.md) **§10**.
...
- **Manual:** Viewport **≤900px** (`layout-mobile` on `<html>`). Confirm:
  ...
  - Open account menu: dropdown is about **half** the welcome/menu width, **right-aligned**; actions still tappable.
