...
- **Automated (unit):** [`tests/unit/layout-mode-and-viewport.test.ts`](../../tests/unit/layout-mode-and-viewport.test.ts) covers:
  ...
  - `mobile-layout.css`: global nav under `.layout-mobile` — **grid** `.unified-header` (**`repeat(3, minmax(0, 1fr))`**, two rows: logo | user menu, then three equal view tabs); **`.header-nav-cluster`** and inner wrappers use `display: contents`; **`#newDeckBtn`** hidden; **`.user-menu-toggle`** `justify-content: flex-end`, `width: auto`; **`.user-menu-dropdown`** sizing only (no **`display`** — stays **`none`** until **`.show`**); **`.user-menu-dropdown.show`** → **`display: grid`**, `width: max-content`, viewport-capped **`max-width`**, **`right: 0`**; **`.user-menu-item`** `white-space: nowrap` and compact padding; 44px targets on tabs / menu toggle. See [`MOBILE_DESIGN.md`](../../MOBILE_DESIGN.md) **§10**.
