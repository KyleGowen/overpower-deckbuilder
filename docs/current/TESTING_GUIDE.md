...
- **Unit tests**: Add `*.test.ts` under `tests/unit/`. Use `tests/helpers/` for shared utilities (e.g. `apiClient.ts`, `deckImportTestHelpers.ts`, `deckExportTestHelpers.ts`, `cardHoverModalTestHelpers.ts`, `drawHandKoDimmingTestHelpers.ts`). Large suites are split by behavior: deck-export-comprehensive (7 files using `deckExportTestHelpers`), card-hover-modal (4 files using `cardHoverModalTestHelpers`), draw-hand-ko-dimming (5 files using `drawHandKoDimmingTestHelpers`), deck-import-character (3 files: extract-find, process, overlay-edge), deck-import-mission-event (2 files: extract-find, process). Deck editor MV list + reserve UX: **`deck-editor-mobile-reserve-ui.test.ts`** (jsdom/`vm`; see **`DECK_EDITOR_MOBILE_VIEW.md`**). See [tests/helpers/README.md](../../tests/helpers/README.md).
