...
- **Search Bar Styling**: 
  ...
  - **Locations name and special ability header filters**: 480px width with centered alignment (`card-tables.css`: `data-column="name"` and `data-column="special_ability"`)
...
- **Card database (M2c, `.layout-mobile`):** Rules live in [`public/css/mobile-layout.css`](/public/css/mobile-layout.css) under the **Card database**, **Special Cards tab**, **Aspects tab**, **Universe: Advanced tab**, **Locations** filter shell, **Events** tab, **Missions** tab, **Universe: Teamwork** tab, **Universe: Ally** tab, **Universe: Basic** tab, and **Characters tab — card rows** sections.
  ...
  - **Missions (`#missions-table`):** Desktop keeps **mission-set checkboxes** in **`.missions-checkbox-group`**. **Mobile** (`.layout-mobile` and **`@media (max-width: 900px)`** mirror under **`#database-view #missions-table`**): **`tr.missions-filter-row`** uses the same teal **filter shell** pattern as Aspects (**`--missions-header-shell-bg` / `--missions-header-shell-border`**, **`border-radius: 12px`**, **`padding: 12px`**); **`.missions-filter-clear-th`** is **`display: none !important`** (no Missions-tab Clear button — use **All** in the dropdown, adjust checkboxes on desktop, or global clear); spacer / empty **`th`** remain **hidden** on mobile; **`.missions-checkbox-group`** is **`display: none`** on mobile; **`.missions-mobile-set-row`** is a **flex** row with uppercase **`.missions-mobile-set-label`** (**`color: #4ecdc4`**, same teal as DBV **`data-label`** row headings) and full-width **`#missions-mission-set-filter.missions-mission-set-filter`** (**`flex: 1 1 100%`**, **min-height 44px**, teal border, dark fill). **Tbody:** block **card** **`tr`**, **`td:nth-child(n+3)`** hidden, actions cell **two-column grid** (same as Aspects), art tokens **`--dbv-mobile-missions-portrait-img: min(100%, 870px)`** etc., **`max-height: none !important`** on portrait **`img`**. **Caption** (when **`missionUseMobileListArt()`** in **`displayMissions`**): **`.characters-mobile-card-caption__name`**, **`__mission-set`** (mission story set), **`__set-line`** (**`dbvSetCaptionLineFromCard`** — translated product set + **`set_number`**). **`data-dbv-lightbox-context="mission"`** on list **`img`**. **Desktop-only:** [`public/css/database-view.css`](/public/css/database-view.css) **`#missions-table .missions-mobile-set-row { display: none; }`**. **JS:** **`populateMissionsMissionSetSelect`** / **`applyMissionFilters`** / **`missionsFilterUsesMobileSelect`** in [`public/js/search-filter-functions.js`](/public/js/search-filter-functions.js); **`loadMissions`** refreshes the select then applies filters ([`public/js/card-data-display.js`](/public/js/card-data-display.js)).
  ...
  - **Universe: Ally (`#ally-universe-table`):** Same **mobile filter shell** visual pattern as Teamwork **icon row** only (no To Use numeric row). Desktop hides **`th.ally-filter-controls-th`**, **`.ally-mobile-filter-shell`**, and **`#clear-ally-filters-mobile-inline`** in [`public/css/database-view.css`](/public/css/database-view.css) (**`display: none`**, no **`!important`**). **`tr.ally-filter-row`**: teal shell (**`--ally-header-shell-*`**), **`.ally-filter-icon-row`** with **`.ally-stat-type-to-use-toggles`** (**same six **`data-power-type`** toggles as Teamwork**) filtering **`stat_type_to_use`**; **`#clear-ally-filters-mobile-inline`** (**.`clear-filters-btn--ally-mobile-inline`**). **Tbody:** card rows (**`td:nth-child(n+3)`** hidden), **`--dbv-mobile-ally-portrait-img`**, **`max-height: none !important`** on portrait **`img`**. **Caption** (**`buildAllyMobileCaptionHtml`**): **`.characters-mobile-card-caption__ally-name`** (bold **1.125rem**); **`.characters-mobile-card-caption__ally-stat-line`** = **`renderTeamworkValueCell(stat_to_use, stat_type_to_use)`** + literal **` - Acts as `** (in **`.characters-mobile-card-caption__ally-stat-sep`**, **`color: #fff`**) + **`renderTeamworkValueCell(attack_value, attack_type)`**; **`.characters-mobile-card-caption__ally-text`** (**1rem**, not bold) for **card text**; **`.characters-mobile-card-caption__ally-set-line`** from **`dbvSetCaptionLineFromCard`**. **`data-dbv-lightbox-context="ally-universe"`**. **JS:** **`window.allyUniverseData`** in **`loadAllyUniverse`**; **`applyAllyUniverseFilters`** / **`setupAllyUniverseTableFilters`** / **`setupAllyUniverseSearch`** ([`public/js/search-filter-functions.js`](/public/js/search-filter-functions.js)); **`clearAllyUniverseFilters`** ([`public/js/filter-functions.js`](/public/js/filter-functions.js)). **Tests:** [`tests/unit/dbv-ally-mobile.test.ts`](/tests/unit/dbv-ally-mobile.test.ts).
  - **Universe: Basic (`#basic-universe-table`):** Desktop **`tr.basic-universe-desktop-filter-row`** keeps per-column type toggles and value/bonus inputs; **`.layout-mobile`** hides that row and shows **`tr.basic-universe-filter-row`** (Training-style **`basic-universe-stat-type-toggles`** + **`#clear-basic-universe-filters-mobile-inline`**) plus two Teamwork-style numeric shells (**`.basic-universe-to-use-filters-inner`**, **`.basic-universe-bonus-filters-inner`**) with placeholders **To Use** / **Min To Use** / **Max To Use** and **Bonus =** / **Min Bonus** / **Max Bonus**. Desktop hides **`th.basic-universe-filter-controls-th`** and **`.basic-universe-mobile-filter-shell`** in [`public/css/database-view.css`](/public/css/database-view.css). Mobile/desktop inputs stay in sync in **`setupBasicUniverseSearch`** ([`public/js/card-data-display.js`](/public/js/card-data-display.js)). **`window.basicUniverseData`**; **`applyBasicUniverseFilters`** ([`public/js/card-filter-toggles.js`](/public/js/card-filter-toggles.js)); **`clearBasicUniverseFilters`** ([`public/js/filter-functions.js`](/public/js/filter-functions.js)). **Tbody:** **`--dbv-mobile-basic-universe-portrait-img`**, **`max-height: none !important`** on portrait **`img`**. **Caption:** **`.characters-mobile-card-caption--basic-universe`** — **`.characters-mobile-card-caption__basic-universe-name`** (**1.35rem** bold), **`.characters-mobile-card-caption__basic-universe-stat-line`** (type icon + value + bonus), **`.characters-mobile-card-caption__basic-universe-set-line`** from **`dbvSetCaptionLineFromCard`**. **`data-dbv-lightbox-context="basic-universe"`**. **Doc:** [`docs/current/DBV_BASIC_UNIVERSE_MOBILE.md`](DBV_BASIC_UNIVERSE_MOBILE.md). **Tests:** [`tests/unit/dbv-basic-universe-mobile.test.ts`](/tests/unit/dbv-basic-universe-mobile.test.ts).
  - **Events (`#events-table`):** Same **mobile filter shell** pattern as Missions: **`tr.events-filter-row`**, **`.events-mobile-set-label`** (**`#4ecdc4`**), then **`.events-mobile-set-select-clear-row`** (**CSS grid** **`grid-template-columns: 8fr 2fr`**, **~80% / ~20%**) holding **`#events-mission-set-filter`** and **`#clear-events-filters-mobile`** on one row; **`.events-checkbox-group`** hidden on mobile; next full-width row is **`data-column="game_effect"`** (**min-height 44px**). **`.events-filter-clear-th`** (desktop **Clear All Filters**) hidden on mobile. **Tbody** matches **Locations** (block **`tr`**, **`td:nth-child(n+3)`** hidden, **`--dbv-mobile-table-portrait-img`**, **`applyDbvHorizontalCardClass`**, no art nav). The **actions** **`<td>`** in **`displayEvents`** has **no** **`data-label`** so mobile does **not** show the teal uppercase **Actions** row heading (**`td[data-label]::before`**). **Caption** under art: **`__name`** (bold **`1.0625rem`**), **`__set`** (mission set **`1rem`**), **`__game-effect`** (**`1rem`**, **`margin-top: calc(0.5 * 1.35em)`** after mission set — half a line), **`__flavor`** (italic **`0.875rem`**; a **centered** short rule via **`::before`**: **`width: 20%`**, **`margin: 0 auto 10px`**, **`border-top: 1px solid rgba(78, 205, 196, 0.35)`**, after **`margin-top: 10px`** on the block). **Desktop:** **`#events-table .events-mobile-set-row`** and mobile clear hidden in **`database-view.css`**. **JS:** **`populateEventsMissionSetSelect`**, **`applyEventsFilters`** (intersects main search + game effect + mission set), **`loadEvents`** in [`public/js/card-data-display.js`](/public/js/card-data-display.js); **`clearEventsFilters`** resets search, game effect, checkboxes, and select ([`public/js/filter-functions.js`](/public/js/filter-functions.js)).
  - **Special Cards table:** `#special-cards-table` column 2 `min-width` cleared for scroll (shared with other tables). **Mobile:** **`thead`** **`display: block`**, **`tr.special-cards-filter-row`** **`flex-direction: row`** + **`flex-wrap: wrap`** + **`justify-content: center`** (Characters-like shell). **Two centered rows:** function toggles (**full-width** **`th`**, **`order: 1`**); then power/type + No Icon (**full-width** **`th`**, **`order: 2`**). **`.special-function-filter-toggles`** / **`.special-power-filter-toggles`**: **`justify-content: center`**, **~50px** toggles, **~34px** icons. **character / name / effect / value** **`th`** use **`flex: 1 1 100%`** and **`width/max-width: 100% !important`** to override **`database-view.css`** per-column **`width: … !important`** (avoids a narrow left “rail”). **Filter visual order** (CSS **`order`**): function row (**toggles**, teal **`|`**, compact **`#clear-special-filters-mobile`** “Clear filters” in **`.special-function-mobile-trailing`**) → type + No Icon → value row (**`.special-value-inputs-and-clear`**: horizontal **`padding: 0`**; **`.column-filters`** **grid** **`4fr 85fr 4fr 125fr 4fr 125fr 4fr 45fr 4fr`** — **1%** buffers/gutters, **21.25 / 31.25 / 31.25 / 11.25%** for **`=`** / Min / Max / No value (space reclaimed from halving **2%→1%** gutters); edges align with **`.header-filter`** via **`width/max-width: 100% !important`** + **`margin: 0`**, overriding **`card-tables.css`** **85%**) → character → card name → card text. **`#clear-special-filters-desktop`** hidden on mobile. **Tbody:** card **`tr`** blocks, **`td:nth-child(n+3)`** hidden, portrait art **`width: var(--dbv-mobile-special-portrait-img)`** with **`--dbv-mobile-special-portrait-img: min(100%, 870px)`** (fills the card cell without **`100vw`** overflow vs shell padding; **`overflow: visible`** on height-locked **`tr`/`td`** on mobile avoids clip); **with-nav** portrait **`flex: 1 1 0`** between arrows; **`max-height: none !important`** overrides desktop **`td img { max-height: 180px !important }`** from `database-view.css`; landscape **`horizontal-card`** + **`--dbv-mobile-special-tile-img-max`** (**`100%`**) / **`--dbv-mobile-special-tile-img-landscape-max-h`**, **44px** nav arrows, actions grid. **Hover / lightbox (layout-mobile):** **`data-card-type='special'`** hover caps and **`#imageModal[data-open-context='special']`** (**`width`/`max-width`** **`min(600px, calc(100vw - 32px))`** on **`#modalImage`** so art scales to the modal frame width) (from **`data-dbv-lightbox-context="special"`** on the list **`img`**, **`openModal`** in **`modal-ui.js`**). **`@media (max-width: 900px)`** repeats tbody art + those hover/lightbox rules for **`#database-view #special-cards-table`** so **`preferDesktopLayout`** on a phone still gets large art. **Caption:** **`specialMobileCaption()`** — stacked lines: card **name** (**`__name`** **~1.0625rem** bold), **character** (**`__character`** **1rem**), plain **card text** (**`__ability`** **0.875rem**; keyword tokens stripped; OPD/Cataclysm/Assist/Ambush come from DB flags as separate lines), optional **One per deck** / **Cataclysm** / **Assist** / **Ambush**, then **`{set label} - {number}`** (spaces around **`-`**). Desktop: **`.special-function-mobile-trailing`** **`display: none`** in [`public/css/database-view.css`](/public/css/database-view.css).
