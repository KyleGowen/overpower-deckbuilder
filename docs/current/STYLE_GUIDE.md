...
- **Thumb-drag accordion (primary on touch):** **`pointerdown`** / **`pointermove`** / **`pointerup`** / **`pointercancel`** on **`#drawHandContent`** (when **`draw-hand-active`**) with **`setPointerCapture`**: touch down on a card sets **`.draw-hand-peek-active`** (magnify); vertical drag past ~**48px** per step moves focus to the adjacent card (up → lower **`dataset.index`**, down → higher); lifting the finger clears peek. **`draw-hand-fan-dragging`** on the content zeros transform transitions while dragging. **`suppressDrawHandClickUntil`** avoids synthetic **`click`** fighting the gesture. **`.drawn-card`** is **`draggable: false`** on mobile so HTML5 drag does not compete with the fan; desktop keeps reorder drag.
