...
- **Card database (M2c, `.layout-mobile`):** Rules live in [`public/css/mobile-layout.css`](/public/css/mobile-layout.css) under the **Card database**, **Special Cards tab**, **Universe: Advanced tab**, and **Characters tab — card rows** sections.
  ...
  - **Universe: Advanced (`#advanced-universe-table`):** Under **`.layout-mobile`**, follows the same **card-row + filter-shell** pattern as **Special Cards**. **`thead`** **`display: block`**, first header row **visually hidden**, **`tr.advanced-universe-filter-row`** flex wrap with the same **shell** border/radius/background tokens as Special filters. **Mobile filter order:** full-width **Clear** (**`advanced-filter-clear-th`**) → **`data-column="character"`** → **`data-column="card_effect"`** (placeholder **“Search card text…”**); **`advanced-filter-spacer-th`** and empty **`advanced-filter-opd-th`** hidden. **Tbody:** block **card** **`tr`**, **`td:nth-child(n+3)`** hidden, **`.card-image-container`** + **`.characters-mobile-card-caption`** (**`__name`**, **`__character`**, **`__ability`** from **`formatAdvancedUniverseCardEffect`**, **`__opd`** **“One per deck: Yes/No”**), **`--dbv-mobile-special-*`** art tokens, **`max-height: none !important`** on portrait **`img`**, **`applyDbvHorizontalCardClass`** after render, **`data-dbv-lightbox-context="advanced-universe"`**. **`@media (max-width: 900px)`** mirrors tbody + image rules under **`#database-view #advanced-universe-table`**. Markup: [`public/index.html`](/public/index.html); **`displayAdvancedUniverse`**: [`public/js/card-display-functions.js`](/public/js/card-display-functions.js).
