...
- **Panel**: `#deckEditorControlsMenuPanel` (`.deck-editor-controls-menu-panel`) — default **`position: absolute`**, `top: calc(100% + 8px)`, `right: 0`, `z-index: 10051`, **`min-width: min(198px, calc(100vw - 48px))`**, scrollable (`max-height: min(70vh, 520px)`), dark gradient fill, teal border `rgba(78, 205, 196, 0.4)`, radius `10px`, elevated shadow. When **`layout-mobile`** or **`max-width: 480px`**, and the menu is **open** (`.deck-editor-controls-menu.deck-editor-controls-menu-open`), the panel uses **`position: fixed`**, **`left: 50%`**, **`translateX(-50%)`**, **`width: min(280px, calc(100vw - 32px))`** so **`left: 50%`** is relative to the **viewport** (not the narrow menu shell). **`top`** is set in JS (**`positionDeckEditorControlsMenuPanel`** in [`deck-editor-core.js`](/public/js/deck-editor-core.js) via **`getBoundingClientRect()`** under the toggle). **`:not(.dev-mobile-deck-row-menu-panel)`** leaves the card-row ⋯ flyout unchanged.
