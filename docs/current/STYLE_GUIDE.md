...
- **Special Cards Table Columns**:
  ...
  - **Full filter behavior**:
    ...
    - Value filters: `=` / `Min` / `Max` numeric inputs; **Special Cards** “No value”: hidden **`#special-no-value-toggle`** + **`.special-no-value-toggle-face`** / **`.special-no-value-svg`** (ban icon, same chrome as No Icon). **Aspects** keep checkbox + “No value” text. NULL-only filter still disables numeric inputs.
...
- **Card database (M2c, `.layout-mobile`):** Rules live in [`public/css/mobile-layout.css`](/public/css/mobile-layout.css) under the **Card database**, **Special Cards tab**, and **Characters tab — card rows** sections.
  ...
  - **Missions:** `#missions-table` mission-set and game-effect cells and `#missions-table .checkbox-group` min-widths are relaxed.
