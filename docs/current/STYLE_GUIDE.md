...
- **Single search (MV):** There is **no** second text field inside the glass panel — finding cards to add uses only **`#collectionSearchInput`** + **`CardSearchService`** / **`DeckEditorSearch`** above the panel. The mobile list is for browsing owned/unowned rows only (**`sortMergedCollectionCards(..., 'set_number', 'asc')`**); desktop column sort does not change MV order.
