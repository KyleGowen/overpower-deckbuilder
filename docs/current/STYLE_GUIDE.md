...
- **Primary controls**: `.app-tab-button` uses **`min-height: 35px`**, **`padding: 5px 3px`**, **`font-size: 12px`**, **`inline-flex`** centered label; `.user-menu-toggle` **`min-height: 44px`**, **`justify-content: flex-end`**, **`font-size: 14px`** (MV welcome stacks **`.user-greeting-line1`** / **`.user-greeting-line2`**). **`.app-tabs`** **`padding-inline: 5%`** narrows the tab group slightly.
