...
| G.D.A. Any Character specials | Global Defense Agency Battleground text permits its G.D.A. Any Character subset | `gda_any_character_requires_battleground` — Skybound collectors **363–374** require the **Global Defense Agency** Battleground; `gda_any_character_exclusive` forbids mixing them with other Any Character Specials | Not checked | Add Cards Hide Unusables mirrors both dependency and exclusivity; live/full legality comes from `POST /api/v1/decks/validate` |
