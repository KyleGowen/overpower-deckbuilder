...
| Power cards usable | Grid / type (~L65–L71) | `unusable_power` — DB `power_type` / `value`; Any-Power / Multi-Power / Multi Power use max grid | Not checked | **Aligned:** client uses `power_type` + `value` |
