...
describe('mobile-layout.css (M1 shell)', () => {
    it('includes global nav stacking and 44px touch targets under .layout-mobile', () => {
        ...
        expect(css).toMatch(/Global nav MV: grid \(logo \| 1fr tabs \| user\)/);
        expect(css).toMatch(
            /\.layout-mobile\s+\.unified-header[\s\S]*?display:\s*grid[\s\S]*?grid-template-columns:\s*auto\s+1fr\s+auto/
        );
        ...
        expect(css).toMatch(
            /\.layout-mobile\s+\.header-nav-cluster\.collection-tab-hidden\s+#deckBuilderBtn[\s\S]*?flex:\s*2\s+1\s+0/
        );
        expect(css).toMatch(
...
describe('mobile-layout.css (DBV Characters tab)', () => {
    ...
    it('shows the mobile caption stack with name, ability, and set line font sizes', () => {
        expect(css).toMatch(
            /#characters-table\s+\.characters-mobile-card-caption\s*,\s*#special-cards-table\s+\.characters-mobile-card-caption\s*,\s*#locations-table\s+\.characters-mobile-card-caption\s*,\s*#aspects-table\s+\.characters-mobile-card-caption\s*,\s*#events-table\s+\.characters-mobile-card-caption\s*,\s*#advanced-universe-table\s+\.characters-mobile-card-caption\s*\{[\s\S]*?display:\s*none/
        );
        expect(css).toMatch(
