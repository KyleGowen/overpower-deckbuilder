...
describe('Cross-User Mutation Attempts Integration Tests', () => {
  ...
  it('blocks non-owners from mutating another user’s deck (403)', async () => {
    ...
    expect((await request(app).put(`/api/decks/${deckId}`).set('Cookie', cookieB).send({ name: 'Hacked' })).status).toBe(403);

    expect((await request(app).post(`/api/decks/${deckId}/cards`).set('Cookie', cookieB).send({ cardType: 'character', cardId: testCharacterId, quantity: 1 })).status).toBe(403);

    expect((await request(app).put(`/api/decks/${deckId}/cards`).set('Cookie', cookieB).send({ cards: [{ cardType: 'character', cardId: testCharacterId, quantity: 1 }] })).status).toBe(403);
