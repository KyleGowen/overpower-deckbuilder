/**
 * Integration tests for foil handling in Deck Editor Card View.
 *
 * Verifies:
 * 1. Deck API returns foil card IDs when deck has foil cards
 * 2. Foil-card-map API returns mappings
 * 3. Deck editor page loads foil-related scripts for card view (foil-shimmer, foil-once)
 *
 * Note: Foil UI (foil-btn, foil-shimmer.foil-once) is rendered by client-side JS.
...
describe('Foil Deck Editor Card View Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    testUser = await userRepository.createUser(
      `foil-card-${timestamp}`,
      `foil-card-${timestamp}@example.com`,
      'password123',
      'USER'
    );
    ...
    const deck = await deckRepository.createDeck(
      testUser.id,
      'Foil Card View Test Deck',
      'Deck for foil card view tests'
    );
    testDeckId = deck.id;
  ...
  it('should load deck editor with foil scripts for card view layout', async () => {
